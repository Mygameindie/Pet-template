...
(() => {
  ...
  const baseSets = [
    ...
    { normal: createImg("base_3.png"), hurt: createImg("base_disgust_3.png") },
  ];
  ...
  const pets = Array.from({ length: NUM_PETS }, (_, i) => ({
    ...
    hurtUntil: 0,
    recoilUntil: 0,
    drawFilter: fallbackFilters[i] || "none",
  }));
  ...
  function resizeCanvas() {
    ...
    pets.forEach((p, i) => {
      p.x = canvas.width * ((i + 1) / (NUM_PETS + 1)) - p.w / 2;
      p.y = groundY - 500;
    });
  }
  ...
  // ===========================================================
  const alphaMasks = Array.from({ length: NUM_PETS }, () => ({ data: null, w: 0, h: 0 }));
